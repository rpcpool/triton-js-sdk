...
export class AccountSyncConnection extends Web3JsConnection {
   ...
   * account-sync endpoint unless `accountSync.subscriptionEndpoint` is set.
   * @param commitmentOrConfig Default commitment or a web3.js connection
   * configuration extended with `accountSync` options.
   * @throws `Error` when an endpoint or account-sync option is invalid, when an
   * initial account address is invalid, or when gRPC options are used with the
   * WebSocket transport.
   */
